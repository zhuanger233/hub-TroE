...
def _get_embed_client() -> OpenAI:
    """懒初始化 Embedding 客户端，只有调用 RAG 时才要求 DASHSCOPE_API_KEY。"""
    ...
    if not api_key:
        raise RuntimeError("缺少 DASHSCOPE_API_KEY，无法调用 rag_search 的 DashScope Embedding")
...
def _load_rag():
    ...
    try:
        ...
        # faiss.read_index 在 Windows 上可能无法处理中文路径，改用字节反序列化。
        index_bytes = np.fromfile(index_path, dtype="uint8")
...
def _embed_query(text: str) -> np.ndarray:
    ...
    resp = _get_embed_client().embeddings.create(model=EMBED_MODEL, input=[text])
